...
> 2. Intel 公布 B580 @2.85 GHz 为 117 TF/s $\Rightarrow \frac{117\text{e}12}{20\times2.85\text{e}9}=2052\approx 2048$ ，与 2048 FLOP/clk/core 一致。
...
>
> | tile | $T$ | $T \bmod 20$ | 末波占用 | 调度代价 |
> |---|---:|---:|---:|---:|
> | $256\times256$ | 64 | **4** | 4/20 | **1.250** |
> | $64\times64$ | 1024 | **4** | 4/20 | **1.016** |
>
> **余数完全相同，代价差 15 倍。** 所以剪枝时先看 $T$ 够不够大，
> 别去凑 $T \bmod 20 = 0$ —— 那只在 $T$ 本来就小的时候才值得管。
>
> **但 tile 变细会同时压小 $D$**（见第二栏），固定指令开销摊不薄：
> $64\times64$ 的 instr/dpas 是 3.11，比 $256\times128$ 的 2.26 更差，
> 只是调度改善盖过了指令劣化（实验 8b）。两者方向相反，要权衡。
...
- **调度代价** —— 同样**不用 profiler**，也该进剪枝：**优先把 $T$ 做大**，
  整除 20 只是锦上添花（原因见下面的说明框）。
